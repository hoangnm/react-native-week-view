...
First you should check if the props `eventContainerStyle` and `eventTextStyle` are enough for your use case.

:::

Provide your custom component with the [`EventComponent` prop](../full-api/week-view-props#eventcomponent) for regular events, and the [`AllDayEventComponent` prop](../full-api/week-view-props#alldayeventcomponent) for all-day events. The component will be rendered inside a `TouchableOpacity` with background color set to `event.color` and placed with absolute position in the grid. The component receives the `event` as prop.
