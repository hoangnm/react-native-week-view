...
See the [_Guides_](./category/guides) section for more examples.
...
<WeekView
  onEventPress={event => {
    Alert.alert(`Pressed event: ${event.color} - ${event.id}`);
  }}
  ...
  onDayPress={(date, formattedDate) => {
    Alert.alert(`Pressed day: ${formattedDate}`);
  }}
  onMonthPress={(date, formattedDate) => {
    Alert.alert(`Pressed month: ${formattedDate}`);
  }}
/>
```
