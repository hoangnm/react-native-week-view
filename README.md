...
* **`goToDate(date, {animated = true, left: number = null})`**: navigate to a custom date. If `left = value` is provided, the target `date` will appear at `value` days from the left (e.g. use `left = 0` to show `date` at the most left). Note: `left` will require using `allowScrollByOneDay={true}` in most cases.
