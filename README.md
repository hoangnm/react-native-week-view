...
* **`isRefreshing`** _(Boolean)_ - If `true`, the component will show an `ActivityIndicator` in the middle of the grid.
* **`RefreshComponent`** _(React.Component)_ - Custom component used when `isRefreshing` is `true`. Receives a `style` prop that must be passed down (indicates the component position), for example: `MyRefreshControl = ({ style }) => <Text style={style}>loading...</Text>`
